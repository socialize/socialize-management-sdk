...
class ManagementBase(object):
    base_management_path = 'management'
    version = 'v1'
    management_endpoints = {
            'smart_alert' : 'smart_alert',
            }                    

    management_endpoint_verb = {
            }

class CollectionBase(ManagementBase):
    def _request(self, endpoint, params={}):
        """
            simple request - response with out parsing any content.
        """
        request_url = '%s/%s/%s/%s/'%(self.host,
                                self.base_management_path,
                                self.version,
                                self.management_endpoints[endpoint])
    ...
    def _find(self, endpoint, params={}, verb=None):
        ...
        request_url = '%s/%s/%s/%s/'%(self.host,
                                ...
                                self.management_endpoints[endpoint])
        if verb:
            if verb in self.management_endpoint_verb[endpoint]:
...
class ObjectBase(ManagementBase):
